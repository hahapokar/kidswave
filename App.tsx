    ...
    <div className="min-h-screen bg-[#fafafa] font-sans text-neutral-900">
      <header className="fixed top-0 w-full z-40 bg-white/80 backdrop-blur-xl border-b border-neutral-100 h-24 flex items-center justify-between px-8 md:px-16">
        <div className="cursor-pointer" onClick={() => setShowDesignerPage(true)}>
          <h1 className="text-2xl font-black tracking-tighter serif-font">KIDSWAVE</h1>
        ...
        {showDesignerPage ? (
          <section className="animate-in fade-in duration-1000">
            <div className="relative h-[85vh] flex flex-col items-center justify-center text-center px-6">
              <div className="w-40 h-40 md:w-56 md:h-56 rounded-full overflow-hidden mb-12 shadow-2xl border-[6px] border-white">
              ...
              <button onClick={() => setShowDesignerPage(false)} className="group relative px-12 py-5 bg-black text-white text-xs font-bold tracking-[0.4em] uppercase rounded-full shadow-2xl hover:scale-110 transition-transform">
          ...
          <section className="max-w-7xl mx-auto px-6 py-12 animate-in slide-in-from-bottom-8 duration-700">
            <div className="flex flex-wrap gap-3 mb-20 justify-center">
              {['ALL', ...Object.values(Category)].map(cat => (
                <button key={cat} onClick={() => setSelectedCategory(cat as any)} className={`px-8 py-3 text-[10px] tracking-widest uppercase border-b-2 font-bold ${selectedCategory === cat ? 'border-black text-black bg-neutral-100' : 'border-transparent text-neutral-400 hover:text-neutral-700'}`}>
                  {cat === 'ALL' ? (lang === 'zh' ? '全部品类' : 'All') : cat}
                </button>
              ))}
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10">
              {filteredItems.map(item => (<PortfolioCard key={item.id} item={item} onClick={handleCardClick} lang={lang} />))}
            </div>
          </section>
        )}
      </main>
      ...
      {selectedItem && !showImagePasswordPrompt && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 md:p-12 overflow-y-auto">
          <div className="bg-white w-full max-w-6xl rounded-3xl overflow-hidden flex flex-col md:flex-row relative shadow-2xl">
            ...
            <div className="md:w-3/5 bg-neutral-100">
              <WatermarkedImage src={imagePasswordUnlocked.has(selectedItem.id) ? (selectedItem.originalImage || selectedItem.coverImage) : selectedItem.coverImage} alt={selectedItem.title} className="w-full h-full" isSemiPublic={selectedItem.visibility === 'SEMI_PUBLIC' && !imagePasswordUnlocked.has(selectedItem.id)} blurPercentage={selectedItem.blurPercentage} />
            </div>
            <div className="md:w-2/5 p-10 md:p-14 flex flex-col justify-between bg-white">
              <div className="space-y-10">
                ...
                <div className="space-y-6 text-lg text-neutral-800 font-normal leading-relaxed">
                  <p><strong className="text-black font-bold mr-2">{APP_TRANSLATIONS.designDescription[lang]}</strong>{lang === 'zh' ? selectedItem.designInspiration : ((selectedItem as any).designInspiration_en || selectedItem.designInspiration)}</p>
              ...
              <button onClick={() => {
                ...
                }} className="w-full py-8 bg-black text-white font-bold text-lg uppercase tracking-widest mt-10 rounded-xl shadow-xl hover:bg-neutral-800 transition-all">
      ...
      {showContactPage && (<ContactPage onClose={() => { setShowContactPage(false); setInitialContactMessage(''); }} lang={lang} initialMessage={initialContactMessage} />)}
      ...
      {showAdminLogin && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/95 p-6 backdrop-blur-md">
          ...
          }} className="bg-white p-16 rounded-3xl max-w-sm w-full text-center space-y-8">
            <h3 className="font-bold text-xl tracking-tighter uppercase">Studio Admin</h3>
            <input type="password" value={adminPassword} onChange={e => setAdminPassword(e.target.value)} placeholder="PASSWORD" className="w-full p-5 bg-neutral-100 rounded-2xl text-center outline-none focus:ring-2 focus:ring-black font-mono tracking-widest text-black" autoFocus />
            <button type="submit" className="w-full py-5 bg-black text-white font-black text-xs tracking-widest rounded-2xl">AUTHORIZE</button>
      ...
      <footer className="py-24 text-center border-t border-neutral-100 bg-white mt-24">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-12 items-center text-left">
          <div className="space-y-4"><h4 className="text-2xl font-black serif-font">KIDSWAVE</h4><p className="text-xs text-neutral-400 max-w-sm">重塑儿童审美的边界。通过极简主义设计语言为成长注入高级质感。</p></div>
          <div className="flex flex-col md:items-end space-y-6"><p className="text-[9px] text-neutral-300 tracking-[0.4em] uppercase">© 2026 KIDSWAVE STUDIO PORTFOLIO</p><button onClick={() => setShowAdminLogin(true)} className="text-[9px] font-bold text-neutral-200 hover:text-neutral-900 transition-colors uppercase">[ Maintainer Entrance ]</button></div>
        </div>
      </footer>
    </div>
  );
